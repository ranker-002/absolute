...
export class GitIntegration {
  ...
  private async initRepo(): Promise<void> {
    try {
      await execAsync('git init && git add -A && git commit -m "Initial commit: ULTIMATE bootstrap"');
      this.isRepo = true;
  ...
  async commit(message: string, files?: string[]): Promise<GitCommitResult> {
    ...
    try {
      ...
      const { stdout } = await execAsync(`git commit -m "${message}" --allow-empty`);
      const hashMatch = stdout.match(/\[[\w]+ ([a-f0-9]+)\]/);
      const commitHash = hashMatch?.[1] || '';

      logger.info('Git', `Committed: ${message} (${commitHash.substring(0, 7)})`);
  ...
  async getLog(limit = 10): Promise<Array<{ hash: string; message: string; date: string }>> {
    ...
    try {
      const { stdout } = await execAsync(`git log --oneline -${limit} --format="%H|%s|%ai"`);
